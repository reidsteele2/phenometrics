...
#' @param ... Additional arguments to feed to ToM functions (`ks_tom`, `emp_tom`)
#'
...
comm_mismatch = function(data,
                         ...
                         ... # Additional arguments to feed to ToM functions
){
    ...
    for(j in 1:nrow(tests)){
      ...
      if((sp2_ori$mean >= min(sp1_lims)) & (sp2_ori$mean <= max(sp1_lims))){
        ...
        if(method == 'empirical'){
          ...
          # Run mismatch test
          mm = emp_tom(sp1_data, sp2_data, plot = F, ...)
        ...
        if(method == 'statistical'){
          ...
          # Run mismatch test
          mm = ks_tom(sp1_data, sp2_data, plot = F, ...)
